...
class HrEmployeePublic(models.Model):
    ...
    attendance_break_state = fields.Selection(
        related="employee_id.attendance_break_state",
        readonly=True,
        groups="taerp_records_rrhh.group_hr_attendance_kiosk,"
               "taerp_records_rrhh.group_hr_attendance",
    )

    last_attendance_break_id = fields.Many2one(
        related="employee_id.last_attendance_break_id",
        readonly=True,
        groups="taerp_records_rrhh.group_hr_attendance_kiosk,"
               "taerp_records_rrhh.group_hr_attendance",
    )
...
class HrEmployee(models.Model):
    ...
    attendance_break_state = fields.Selection(
        ...
        selection=[("break_out", "Break out"), ("break_in", "Break in")],
        store=True,
        groups="taerp_records_rrhh.group_hr_attendance_kiosk,"
               "taerp_records_rrhh.group_hr_attendance",
        compute_sudo=True,  # Ejecuta el método de cálculo con privilegios de superusuario
    )
    ...
    def _compute_last_attendance_id(self):
        for employee in self:
            employee.last_attendance_id = self.env["hr.attendance"].search(
                [("employee_id", "=", employee.id), ("type", "=", "normal")], limit=1
            )
            employee.last_attendance_break_id = self.env["hr.attendance"].search(
                [("employee_id", "=", employee.id), ("type", "=", "break")], limit=1
            )

    @api.depends(
        "last_attendance_id.check_in",
        "last_attendance_id.check_out",
        "last_attendance_break_id.check_in",
        "last_attendance_break_id.check_out",
        "last_attendance_id",
    )
    def _compute_attendance_state(self):
        for employee in self:
            ...
            employee.attendance_break_state = (
                att and not att.check_out and "break_in" or "break_out"
            )
    ...
    def _attendance_break_action_change(self):
        ...
        if attendance:
            ...
            raise exceptions.UserError(
                _(
                    "Cannot perform break out on %(empl_name)s, could not find "
                    "corresponding break in. "
                    "Your attendances have probably been modified manually by "
                    "human resources."
                )
                % {
    ...
    def attendance_break_manual(self, next_action, entered_pin=None):
        ...
        attendance_user_and_no_pin = self.user_has_groups(
            "taerp_records_rrhh.group_hr_attendance_user,"
            "!taerp_records_rrhh.group_hr_attendance_use_pin"
        )
    ...
    def _attendance_break_action(self, next_action):
        ...
        action_message["previous_attendance_change_date"] = (
            employee.last_attendance_break_id
            and (
                employee.last_attendance_break_id.check_out
                or employee.last_attendance_break_id.check_in
            )
            or False
        )
        ...
        if employee.user_id:
            modified_attendance = employee.with_user(
                employee.user_id
            )._attendance_break_action_change()
        else:
    ...
    def attendance_break_scan(self, barcode, type="normal"):
        ...
        if employee:
            if type == "normal":
                ...
                return employee._attendance_break_action(
                    "hr_attendance.hr_attendance_action_kiosk_mode"
                )
        return {
